...
Key differences from Exp 2:
  - proprio_dim = 46 (2 cursor + 1 mouse_left + 43 keys_held)
  - action_dim = 46 (2 dx/dy + 1 mouse_left + 43 keys_held)
  - Mouse and keys are independent binary outputs (sigmoid/BCE),
    not one-hot encoded categories.
...
class ACT(nn.Module):
    ...
    Predicts chunk_size timesteps of (dx, dy, mouse_left, keys_held, pad)
    ...
    def __init__(
        ...
        # 7. Action heads
        self.head_dx = nn.Linear(d_model, 1)                    # regression
        self.head_dy = nn.Linear(d_model, 1)                    # regression
    ...
    def forward(
        ...
        Args:
            ...
            actions: (B, chunk_size, action_dim) or None for inference
                     action_dim = 46: 2 dx/dy + 1 mouse + 43 keys

        Returns:
            dict with dx, dy, mouse_left, keys_held, pad_logits, mu, logvar.
        """
        B = images.size(0)
        ...
        mouse = self.head_mouse(decoded).squeeze(-1)           # raw logit
        keys = self.head_keys(decoded)                          # raw logits
        pad = self.head_pad(decoded).squeeze(-1)               # raw logit

        return {
            "dx": dx,              # (B, chunk)
            "dy": dy,              # (B, chunk)
            "mouse_left": mouse,   # (B, chunk) — raw logit for sigmoid/BCE
            "keys_held": keys,     # (B, chunk, 43) — raw logits for per-key sigmoid/BCE
            "pad_logits": pad,     # (B, chunk) — raw logit for sigmoid/BCE
            "mu": mu,              # (B, latent_dim)
