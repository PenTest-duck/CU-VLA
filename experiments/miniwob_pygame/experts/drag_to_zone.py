"""Expert policy for the drag-to-zone task.

Navigates to each shape, drags it to the matching zone, and drops it.
Uses nearest-first ordering when multiple shapes are present.
"""
...
def generate_trajectory(
    ...
    """Generate expert trajectory for drag-to-zone task.
    ...
    Args:
        ...
        shapes: List of shape dicts with x, y, width, height, color, etc.
...
def run_expert_episode(
    ...
    """Run a full expert episode for drag-to-zone.

    Args:
        env: DragToZoneEnv instance (already reset if seed is None).
